...
int main(int argc, char* argv[]) {
  ...
  struct timespec prof_start, prof_end, prof_graph_start, prof_graph_end;
  ...
  clock_gettime(CLOCK_REALTIME, &prof_end);

  double seconds = (double)((prof_end.tv_sec+prof_end.tv_nsec*1e-9) - (double)(prof_start.tv_sec+prof_start.tv_nsec*1e-9));
  LOG(INFO) << "main time " << seconds << "s";

  seconds = (double)((prof_graph_end.tv_sec+prof_graph_end.tv_nsec*1e-9) - (double)(prof_graph_start.tv_sec+prof_graph_start.tv_nsec*1e-9));
  LOG(INFO) << "graph time " << seconds << "s";
